...
Three design choices therefore compete for the same budget: 

1. How many biological samples to collect 
2. How deeply each sample is measured
3. Whether to measure one or several molecular layers
...
1. **Define the features the question depends on.** This follows from the scientific question and hypothesis established in [Module 1.2.1.](module1-2-1.md) Which molecular layer, which specific features, must be detected for the question to be answered?
...
4. **Determine the minimum biological sample number using a power calculation.** The inputs come from domain knowledge, comparable published studies, or pilot data, as described in [Module 2.2.1.](module2-1-2.md) This is the *n* per group required to detect the effect of interest at the desired power level.
...
1. 2. **Cost compounds.** Each molecular layer carries its own per-sample cost. A study measuring transcriptomics and proteomics on the same samples pays the per-sample cost of both platforms for every sample.
...
The figure below illustrates this across a real multi-omics study spanning six platforms. The required sample number  was set by the most demanding platform, not the average across platforms.

![MultiPower output: per-omic power curves and combined multi-omic sample-size optimisation across six omics platforms in the STATegra dataset](figs/tarazona2020_fig4_MultiPower_v02.jpg){width=90%}
