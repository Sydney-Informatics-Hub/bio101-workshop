...
!!! question "Activity: Selecting a platform"
    ...
    For each scenario below, use the figure to identify the platform you would choose.
    ...
    2. A study wants to understand cell-type-specific transcriptional responses to a drug treatment in a heterogeneous tissue.
    3. A study aims to profile the open chromatin landscape across a developmental time course.
    4. A study wants to measure metabolite profiles in patient serum. What platform would you choose, and what data type will it produce?

    ??? success "Answers"
        1. Bulk RNA-seq: large sample number, tissue-level gene expression.
        2. Single-cell or spatial transcriptomics: cell-type resolution in heterogeneous tissue.
        3. ATAC-seq: measures open chromatin accessibility.
        4. Mass spectrometry (untargeted) or NMR spectroscopy; both produce signal intensities, not sequence reads.

Two patterns run across the figure: 

1. Sequencing-based platforms all produce **reads** and signal-based platforms produce **intensities** — the raw numbers differ in kind, not just in scale. 
2. Within any layer, the choice between platforms turns on **scope** (the breadth of molecules the assay can detect), **resolution** (the biological unit at which signal is measured), or **analyte class** (the specific molecular species the platform can capture).
...
!!! question "Activity: Discovery or targeted?"

    For each scenario below, decide whether a discovery or targeted approach is needed, and identify a platform from the figure that fits.

    1. A study aims to identify unknown metabolites associated with a disease phenotype, with no prior hypothesis about which pathways are involved.
    2. A study needs to quantify the abundance of a predefined panel of ~90 plasma proteins across 200 samples.
    3. A study wants to detect all proteins present in a novel cell type that has never been characterised before.

    ??? success "Answers"
        1. Discovery; untargeted mass spectrometry. No prior targets means the platform must sample broadly across the metabolome.
        2. Targeted; Olink. The protein set is predefined as Olink is explicitly designed for targeted protein panels.
        3. Discovery; mass spectrometry (untargeted proteomics). The cell type is uncharacterised, so no target list exists to design a panel against.
...
!!! tip "Neither label-free nor labelled is universally better"
...
!!! warning "Absent proteins or a technical limit"
    If an ion was never sampled and fragmented during acquisition (see grey examples in top left figure), no downstream processing can identify its presence, identity, or abundance from the raw
...
!!! info "Module 2.1 takeaways"
    ...
    - In mass spectrometry, the choice between DDA and DIA shapes the pattern of missing values in the data.