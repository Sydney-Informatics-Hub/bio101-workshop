...
| Layer | Stability | Tissue specificity | Key constraint |
|---|---|---|---|
| **Genome** | Stable across all cells and time (germline mutations aside) | Same variants present in all tissues; findings from one tissue inform others | Knowing a variant exists does not establish its functional effect in a given tissue — that depends on isoform, chromatin context, and local gene regulation. Somatic mutations differ between tissues and must be measured in the tissue of interest |
| **Epigenome** | Established during differentiation; maintained differently per cell type | Regulatory regions accessible in one cell type are not accessible in another, even though the underlying DNA sequence is identical | An epigenomic result describes regulatory state in that tissue only — it does not describe the same genomic regions in a different tissue |
| **Transcriptome** | Time-sensitive; changes with developmental stage, environment, disease, and treatment | Highly tissue-specific; the same gene may differ substantially between tissues | A result is a snapshot: it describes which genes were active in that tissue, at that time point, under those conditions |
| **Proteome** | Reflects transcription, translation efficiency, post-translational modification, and degradation — none captured by the transcriptome | Tissue-specific; plasma proteomics and tissue proteomics from the same individual measure different things | Sensitive to sample handling: degradation, freeze-thaw, and storage conditions affect detection and apparent abundance |
| **Metabolome** | Fastest-changing layer; concentrations can shift within minutes | Blood metabolite profiles reflect food intake, activity, time of day, and physiological stress at the moment of collection | Pre-analytical standardisation (fasting protocol, collection time, handling) is part of study design |
| **Targeted assay** | — | — | Measures only features specified in advance; produces no information about features outside the panel |
...
There is a genuine trade-off here. Narrowing recruitment reduces biological variability. Lowering variability means effects are more detectable. [Module 2.2.1](module2-2-1.md) established that statistical power increases as within-group variability decreases: the required sample size is lower when groups are homogeneous. The cost is that the narrower the cohort, the fewer organisms or people the finding applies to. Broader recruitment improves generalisability and increases variability, which requires more samples to detect the same effect size and introduces more batch structure to manage. These cannot all be optimised simultaneously under a fixed budget.
...
!!! example "Case study: polygenic risk scores and ancestry"
    ...
    ![](figs/2-3_ancestry.png){width=90%}
...
[Module 2.2.1](module2-2-1.md) established that underpowered studies not only miss true effects, they also detect signals that depend on which specific samples happened to be in the study. At low sample sizes, the detected features are partly determined by which individuals showed the strongest signal in that particular run. A different set of samples from the same population would produce a partly different result.

A study can fail to replicate in an independent cohort for two different reasons: 

1. There is a genuine biological difference between the populations
2. The original finding was not stable.

![](figs/2-3_samplinginstability.png)

This is compounded by the **winner's curse**: in an underpowered study, the findings that do reach significance are those whose effect sizes were inflated by chance. The reported effect is therefore larger than the true underlying effect and a replication study, even an adequately powered one, will typically observe a smaller magnitude and may not replicate at all.
